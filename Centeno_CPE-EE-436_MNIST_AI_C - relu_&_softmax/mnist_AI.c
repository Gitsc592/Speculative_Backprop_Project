...
void backwardPropogate(int correct_label) {
	...
	for (int i = 0; i < LAYER3_N_COUNT; i++) {
		...
		for (int j = 0; j < LAYER4_N_COUNT; j++) {
			gradient_L3[i][j] += output[j].error_der * layer_3[i].activation * softmax_func(j) * (1 - softmax_func(j));
			layer_3[i].error_der += output[j].error_der * layer_3[i].weights[j] * softmax_func(j) * (1 - softmax_func(j));
		}
	}
	...
	for (int i = 0; i < LAYER4_N_COUNT; i++) {
		output[i].biasgrad += output[i].error_der * softmax_func(i) * (1 - softmax_func(i));
	}
...
double softmax_der(int index) {
	...
	return ((denom * num) - (num * num)) / (denom * denom);
}
...
void ApplyGradient() {
	...
	for (int i = 0; i < LAYER4_N_COUNT; i++) {
		output[i].bias -= output[i].biasgrad * LEARNING_RATE;
		output[i].biasgrad = 0;
	}

	for (int i = 0; i < LAYER3_N_COUNT; i++) {
		layer_3[i].bias -= layer_3[i].biasgrad * LEARNING_RATE;
		layer_3[i].biasgrad = 0;
	}

	for (int i = 0; i < LAYER2_N_COUNT; i++) {
		layer_2[i].bias -= layer_2[i].biasgrad * LEARNING_RATE;
		layer_2[i].biasgrad = 0;
	}
