...
// Confusion Matrix
int confusion_matrix[LAYER4_N_COUNT][LAYER4_N_COUNT] = { 0 };
...
void forwardPropogate(int offset_data, int label);
...
int main(int argc, char* argv[]) {
	...
	// Train AI
	// Open training pixel files
	data_file = fopen(Train_Data_File_Pixels, "rb");
	if (data_file == NULL) {
		printf("Cannot open %s\n", Train_Data_File_Pixels);
		exit(2);
	}

	// Open training label files
	label_file = fopen(Train_Data_File_Labels, "rb");
	if (label_file == NULL) {
	...
	for (k = 0; k < EPOCH; k++) {
		for (int i = 0; i < TRAINING_IMAGES; i++) {
			current_label = getLabel(LABEL_START_OFFSET + i);
            ...
            #pragma omp parallel num_threads(2)
			{
				...
				{
					forwardPropogate(PIXEL_START_OFFSET + (i * PIXELS_PER_IMAGE), current_label);
					//printf("The current label is %d\n", current_label);
					//printf("%d\n", i);
				}
	...
	// Open test pixel file
	data_file = fopen(Test_Data_File_Pixels, "rb");
	if (data_file == NULL) {
		printf("Cannot open %s\n", Test_Data_File_Pixels);
		exit(2);
	}

	// Open test label files
	label_file = fopen(Test_Data_File_Labels, "rb");
	if (label_file == NULL) {
	...
	for (int i = 0; i < TEST_IMAGES; i++) {
		...
		//printf("The current label is %d\n", current_label);
		confusion_matrix[current_label][max_neuron]++;
