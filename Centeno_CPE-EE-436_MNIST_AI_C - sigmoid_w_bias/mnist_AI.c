...
void backwardPropogate(int correct_label) {
	...
	for (int i = 0; i < LAYER4_N_COUNT; i++) {
		output[i].biasgrad += output[i].error_der * sigmoid_func(output[i].sum) * (1 - sigmoid_func(output[i].sum));
	}

	for (int i = 0; i < LAYER3_N_COUNT; i++) {
		layer_3[i].biasgrad += layer_3[i].error_der * sigmoid_func(layer_3[i].sum) * (1 - sigmoid_func(layer_3[i].sum));
	}

	for (int i = 0; i < LAYER2_N_COUNT; i++) {
		layer_2[i].biasgrad += layer_2[i].error_der * sigmoid_func(layer_2[i].sum) * (1 - sigmoid_func(layer_2[i].sum));
	}
